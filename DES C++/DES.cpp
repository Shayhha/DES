...
const vector<int> DES::IPMinusOne = {
...
const vector<int> DES::PCMinusOne = {
    57, 49, 41, 33, 25, 17,  9,
     1, 58, 50, 42, 34, 26, 18,
    10,  2, 59, 51, 43, 35, 27,
    19, 11,  3, 60, 52, 44, 36,
...
const vector<int> DES::PCMinusTwo = {
    14, 17, 11, 24,  1,  5,
     3, 28, 15,  6, 21, 10,
    23, 19, 12,  4, 26,  8,
    16,  7, 27, 20, 13,  2,
...
//SBox tables
const vector<vector<vector<int>>> DES::SBoxes = {
    {
...
const vector<unsigned char> DES::InitialPermutation(const vector<unsigned char>& text) {
...
const vector<unsigned char> DES::FinalPermutation(const vector<unsigned char>& text) {
    vector<unsigned char> output(8, 0); //create a new byte array of size 64 bits (8 bytes)

    //goes through IPMinusOne table and initializing the new output array with correct index
    for (size_t i = 0; i < IPMinusOne.size(); i++) {
...
const vector<unsigned char> DES::PCMinusOnePermutation(const vector<unsigned char>& text) {
    ...
    for (size_t i = 0; i < PCMinusOne.size(); i++) {
...
const vector<unsigned char> DES::PCMinusTwoPermutation(const vector<unsigned char>& text) {
    ...
    for (size_t i = 0; i < PCMinusTwo.size(); i++) {
...
const vector<unsigned char> DES::ExpansionPermutation(const vector<unsigned char>& text) {
    vector<unsigned char> output(6, 0); //create a new byte array of size 48 bits (6 bytes)
...
const vector<unsigned char> DES::pPermutation(const vector<unsigned char>& text) {
...
const vector<vector<unsigned char>> DES::KeySchedule(vector<unsigned char> key, const bool isDecrypt) {
...
const vector<unsigned char> DES::XOR(const vector<unsigned char>& text1, const vector<unsigned char>& text2) {
...
const vector<unsigned char> DES::F(const vector<unsigned char>& RightText, const vector<unsigned char>& key) {
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        SboxIndexes = SboxIndexes | (int)(SplitNewRight >> 42 - (i * 6) & 0x3F); // does shifts to right to get the 6 MSB bits
...
const vector<unsigned char> DES::DESFunction(const vector<unsigned char>& text, const vector<unsigned char>& key, const bool isDecrypt) {
...
const vector<unsigned char> DES::Encrypt(const vector<unsigned char>& text, const vector<unsigned char>& key) {
    vector<unsigned char> cipherText = DESFunction(text, key, false); //encrypt the text using key
    return cipherText; //return cipherText
}


const vector<unsigned char> DES::Decrypt(const vector<unsigned char>& text, const vector<unsigned char>& key) {
